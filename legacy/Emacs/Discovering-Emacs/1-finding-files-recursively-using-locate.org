#+TITLE: 1 - Finding Files Recursively Using Locate
#+SUBTITLE: Locate is a program that searches for a specific pattern inside a database that contains pathnames
#+DESCRIPTION: Locate is a program that searches for a specific pattern inside a database that contains pathnames.
#+KEYWORDS: Emacs, Locate
#+SETUPFILE: ./../../setup.org

[[https://www.discovering-emacs.com/2134279/12265945-1-finding-files-recursively-using-locate][Listen To Episode]] | [[https://youtu.be/Qz8O9XWKFwM][YouTube Video]]
...
database is called =updatedb=.

Once the database has been generated, you can use =locate= to find all
...
locations. All you need to do is run =locate pattern= to find matches.
...
installing =findutils=, via brew, instead of trying to use the default
=locate= and =updatedb= commands. The brew version of these commands
will be prefixed with a =g=, so you'll need to run =glocate= and
=gupdatedb=.
...
calls =updatedb= with two arguments, =localpaths= that specifies the
folder that contains all our projects, and =output= that specifies the
...
=LOCATE_PATH=, we can do this inside of our dot Emacs configuration
file.

If your on macOS, you'll also need to set two additional Emacs option
variables, for =locate-command= and =locate-update-command= as we need
to take into account the =g= prefix.
...
A good set of key bindings for these functions will be =C-c f=, to
recursively find files inside the current project. And =C-c F= to update
the locate database.

#+TITLE: 3 - Making Incremental Search Work for You
#+SUBTITLE: Here's some ways I utilize Emacs's incremental search, during my workflow
#+DESCRIPTION: Today I'm going to share some of the ways I utilize Emacs's incremental search, during my workflow. In addition I'll be sharing some configuration recommendations that you might also find useful.
#+KEYWORDS: Emacs, Incremental Search
#+SETUPFILE: ./../../setup.org

[[https://www.discovering-emacs.com/2134279/12359134-3-making-incremental-search-work-for-you][Listen To Episode]] | [[https://youtu.be/9CdbfZXsrqg][YouTube Video]]
...
issues. Setting the option =isearch-lazy-count= to a non nil value, will
...
=M-p=, to advance and =M-n= to retreat. My typical use case will be to
activate incremental search by pressing =C-s= and then immediately
hitting =M-n= repeatedly, until I find the previous search term I'm
looking for.
...
make use of =C-w=, to pull the word that's in front of the cursor into
...
incremental search by pressing =C-s= and then you press =C-w= and the
search term will expand, so it includes the first word. Pressing =C-w= a
...
jumping to other matches by pressing =C-s=.
...
- =C-M-z=, Can be used to yank up until a character. I sometimes use
...
=isearch-yank-on-move= to a non nil value.

#+BEGIN_SRC lisp
(setq-default isearch-yank-on-move t)
#+END_SRC

One other set of key bindings worth mentioning here is =C-y= and =M-y=.
...
previous kill, I can simplify activate incremental search and hit =C-y=.
You can also use =M-y= to cycle through earlier kill ring entries.
...
typing =M-%=. And with the aforementioned yanking capabilities you can
...
first or last match. You can do this by pressing =M-s M-<= to jump to
the beginning and =M-s M->= to jump to the end.

You can also set the option =isearch-allow-motion= to enable the
standard vertical motion key bindings of =M-<= (beginning-of-buffer),
=M->= (end-of-buffer), =C-v= (scroll-up-command) and =M-v=
(scroll-down-command).
...
expression for Occur by pressing =M-s o=. Use this if you need to make
